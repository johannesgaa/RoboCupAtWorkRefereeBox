...
public class Map {
	...
	public static BufferedImage loadBackgroundMap(ConfigFile cfgFile, String pathPrefix) {
		...
		try {
			...
			System.out.println("background file " + backgroundFile.getAbsolutePath() + " exists.");
			return map;
		} catch (Exception e) {
	...
	public static boolean saveTaskSpecMap(File file, MapArea mapArea) {
		...
		try {
			BufferedImage image = new BufferedImage(mapArea.getMapPane().getWidth(), mapArea.getMapPane().getHeight(), BufferedImage.TYPE_INT_RGB);
