...
public class MainController implements TimerListener {
	...
	private Action up = new AbstractAction("Up") {
		...
		@Override
		public void actionPerformed(ActionEvent arg0) {
			if (mG.getCompetitionPanel(compIdent.ordinal()).getSequenceTable().getSelectedRowCount() > 0) {
				int pos = mG.getCompetitionPanel(compIdent.ordinal()).getSequenceTable().getSelectedRow();
				Task t = tS.moveUp(pos, compIdent);
				if (t != null) {
					mG.getCompetitionPanel(compIdent.ordinal()).getSequenceTable().changeSelection(pos - 1, -1, false, false);
	...
	private Action update = new AbstractAction("Update") {
		...
		public void actionPerformed(ActionEvent arg0) {
			...
			if (mG.getCompetitionPanel(compIdent.ordinal()).getSequenceTable().getSelectedRowCount() > 0) {
				int pos = mG.getCompetitionPanel(compIdent.ordinal()).getSequenceTable().getSelectedRow();
				Task t = mG.getCompetitionPanel(compIdent.ordinal()).getSelectedTask();
				Task tUpdate = tS.updateTask(pos, t, compIdent);
				if (tUpdate != null) {
					mG.setStatusLine("Updated Task " + tUpdate.getString() + " to " + t.getString() + ".");
					setUnsavedChanges();
				} else {
	...
	private Action delete = new AbstractAction("Delete") {
		...
		public void actionPerformed(ActionEvent arg0) {
			...
			if (mG.getCompetitionPanel(compIdent.ordinal()).getSequenceTable().getSelectedRowCount() > 0) {
				int pos = mG.getCompetitionPanel(compIdent.ordinal()).getSequenceTable().getSelectedRow();
				String msg = "Do you want to delete the  task " + mG.getCompetitionPanel(compIdent.ordinal()).getSelectedTask().getString() + "?";
	...
	public Action competitionFinished = new AbstractAction("Competition Finished") {
		private static final long serialVersionUID = 1L;
	...
	private void init() {
		save.putValue(Action.SMALL_ICON, new ImageIcon(getClass().getResource("/view/resources/icons/Save16.gif")));
		open.putValue(Action.SMALL_ICON, new ImageIcon(getClass().getResource("/view/resources/icons/Open16.gif")));
		loadConfig.putValue(Action.SMALL_ICON, new ImageIcon(getClass().getResource("/view/resources/icons/Import16.gif")));
		exit.putValue(Action.SMALL_ICON, new ImageIcon(getClass().getResource("/view/resources/icons/exit.png")));
		help.putValue(Action.SMALL_ICON, new ImageIcon(getClass().getResource("/view/resources/icons/Help16.gif")));
		up.putValue(Action.SMALL_ICON, new ImageIcon(getClass().getResource("/view/resources/icons/Up16.gif")));
		down.putValue(Action.SMALL_ICON, new ImageIcon(getClass().getResource("/view/resources/icons/Down16.gif")));
		update.putValue(Action.SMALL_ICON, new ImageIcon(getClass().getResource("/view/resources/icons/Edit16.gif")));
		add.putValue(Action.SMALL_ICON, new ImageIcon(getClass().getResource("/view/resources/icons/Back16.gif")));
		delete.putValue(Action.SMALL_ICON, new ImageIcon(getClass().getResource("/view/resources/icons/Delete16.gif")));
		sendSpec.putValue(Action.SMALL_ICON, new ImageIcon(getClass().getResource("/view/resources/icons/Export16.gif")));
	...
	private void populateValidItems() {
		...
		try {
			if (vte.readFromConfigFile(cfgFile)) {
				...
				((BntPanel) mG.getCompetitionPanel(CompetitionIdentifier.BNT.ordinal())).setValidPositions(vte.getValidBNTPositions());
				((BntPanel) mG.getCompetitionPanel(CompetitionIdentifier.BNT.ordinal())).setValidOrientations(vte.getValidBNTOrientations());
				((BntPanel) mG.getCompetitionPanel(CompetitionIdentifier.BNT.ordinal())).setValidPauses(vte.getValidBNTPauses());

				((BmtPanel) mG.getCompetitionPanel(CompetitionIdentifier.BMT.ordinal())).setValidPositions(vte.getValidBMTPositions());
				((BmtPanel) mG.getCompetitionPanel(CompetitionIdentifier.BMT.ordinal())).setValidConfigurations(vte.getValidBMTConfigurations());
				((BmtPanel) mG.getCompetitionPanel(CompetitionIdentifier.BMT.ordinal())).setValidObjects(vte.getValidBMTObjects());

				((BttPanel) mG.getCompetitionPanel(CompetitionIdentifier.BTT.ordinal())).setValidPositions(vte.getValidBTTPositions());
				((BttPanel) mG.getCompetitionPanel(CompetitionIdentifier.BTT.ordinal())).setValidConfigurations(vte.getValidBTTConfigurations());
				((BttPanel) mG.getCompetitionPanel(CompetitionIdentifier.BTT.ordinal())).setValidObjects(vte.getValidBTTObjects());
				((BttPanel) mG.getCompetitionPanel(CompetitionIdentifier.BTT.ordinal())).setValidSituations(vte.getValidBTTSituations());
        
				((CttPanel) mG.getCompetitionPanel(CompetitionIdentifier.CTT.ordinal())).setValidPositions(vte.getValidCTTPositions());
	...
	private boolean warningIgnored(String actionName) {
		return (mG.getUserConfirmation(unsavedWarningMsg, "Confirm " + actionName) != 0);
	}
	...
	private void loadConfigurationFile(File file) {
		if (cfgFile.setConfigFile(file)) {
			...
			if (initializeBackgroundMap(file.getParent())) {
				...
				mG.setStatusLine("<html><FONT COLOR=RED>Something went wrong!" + "</FONT> No background file loaded. </html>");
			}
		} else {
			mG.setStatusLine("<html><FONT COLOR=RED>Something went wrong!" + "</FONT> No config file loaded. </html>");
		}
	}
